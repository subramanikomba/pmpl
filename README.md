...
## v8-1 (UI: Chemical Stock matrix) — built on the frozen v8 baseline
- Companies & stock → "Chemical stock (per company / location)" redesigned from a repetitive
  one-row-per-(location × chemical) list into a matrix: one row per LOCATION, with grouped
  chemical columns (H2SO4 50% solution: On hand / Low alert; NaOH 100%: On hand / Low alert),
  a light vertical divider between chemicals, and a single Save per location (updates both chemicals).
- Chemical columns are derived from the data (robust if a chemical is added). Locations sorted by name.
